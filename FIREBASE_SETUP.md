...
>
> **Inside the installed app (0.11.1).** Launched from the home screen (standalone) the Google window opens outside the app. It usually works on Android; if the app reports it closed at once (`auth/popup-closed-by-user` within 1.5 s), the game falls back to the redirect, and if that fails too the menu offers **🌐 Open in the browser to sign in**: a Chrome-installed app shares Chrome's storage, so signing in there signs the app in as well. iOS home-screen apps keep their own storage, so that button is not shown there.
>
