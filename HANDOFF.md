...
**Decisions**
* **No precache list.** The shell is cached as it loads (every module is a static import, so one online launch caches the whole game); a list would be one more thing to keep in step and would not make updates safer. On the **very first** load the files are fetched before the worker exists (a live first load had cached 0, and that page was not even controlled yet), so once the worker is ready `js/ui/pwa.js` `warmCache` posts it a `WARM` message with every same-origin file the page loaded (`performance.getEntriesByType('resource')`, without `version.json`) plus the shell, manifest and icons, and `sw.js` fetches and caches them itself: the first online launch already fills the cache (65 files locally).
...
| Update toast (a real deploy) | With the live tab open on `09a767e`, `fb9662e` (the commit that added this section) was pushed and deployed. `checkForUpdate({ force: true })` (what a return to the front or Settings → ↻ Check for updates does) read `version.json` = `fb9662e` and showed the sticky **"Update ready — tap to reload"** button toast. Tapping it reloaded the page: stamp **vfb9662e · 2026-09-25 03:10 UTC** on the menu, the page controlled by the worker, 66 files in the cache, no console messages. |
